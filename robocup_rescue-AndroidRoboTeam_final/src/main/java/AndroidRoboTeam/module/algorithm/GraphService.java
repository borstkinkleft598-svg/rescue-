...
public class GraphService extends AbstractModule {
    ...
    public GraphService(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
            DevelopData developData) {
        super(ai, wi, si, moduleManager, developData);
        areas = new HashSet<>(SEUWorldService.getAreasWithURN(wi));
        roads = new HashSet<>(SEUWorldService.getRoadsWithURN(wi));
        buildings = new HashSet<>(SEUWorldService.getBuildingsWithURN(wi));
    ...
    public List<MyEdge> getMyEdgesBetween(Area from, Area to) {
        ...
        for (MyEdge myEdge : thisAreaMyEdge) {
            if (!myEdgesBetween.contains(myEdge) &&
                    (myEdge.getNodes().first().getNeighbourAreaIds().contains(to.getID())
                            || myEdge.getNodes().second().getNeighbourAreaIds().contains(to.getID()))) {
    ...
    private boolean isOnTooSmallEdge(Area area, Node node) {
        ...
        for (Edge edge : area.getEdges()) {
            if (edge.isPassable()) {
                ...
                if (Ruler.getDistance(edge.getStartX(), edge.getStartY(), edge.getEndX(),
                        edge.getEndY()) < SEUConstants.TOO_SMALL_EDGE_THRESHOLD
