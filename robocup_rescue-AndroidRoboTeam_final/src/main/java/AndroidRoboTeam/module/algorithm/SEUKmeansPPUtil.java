 ...
 * K-means++聚类算法工具类
 * 独立文件，不依赖项目其他模块
 * 仅用于Highways.java的采样功能
 */
public class SEUKmeansPPUtil {
  ...
  public SEUKmeansPPUtil(EntityID[] targets, double[] xs, double[] ys, int n) {
  ...
  public SEUKmeansPPUtil(int n, List<Collection<EntityID>> memberz) {
  ...
  private static Cluster[] init(EntityID[] targets, double[] xs, double[] ys, int n) {
  ...
  private static void assign(Cluster[] clusters, EntityID id, double x, double y) {
    Optional<Cluster> cluster = Arrays.stream(clusters).min(comparing(c -> {
      double cx = c.getCX();
      double cy = c.getCY();
