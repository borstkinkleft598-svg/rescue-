...
public class SEUMessageCoordinator extends MessageCoordinator {
    ...
    public void coordinate(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo,
        ...
        if (scenarioInfo.getCommsChannelsCount() > 1) {
            ...
            List<StandardEntityURN> priority = SEUChannelSubscriber.getPriority(scenarioInfo);
            if (allowedToSendRadioMessage(worldInfo, agentInfo, scenarioInfo)) {
            } else {
                fireBrigadeMessages.removeIf(e -> !(e instanceof CommandPolice || e instanceof CommandFire
                        || e instanceof CommandAmbulance || e instanceof MessageFireBrigade));
                policeMessages.removeIf(e -> !(e instanceof CommandPolice || e instanceof CommandFire
                        || e instanceof CommandAmbulance));
                ambulanceMessages.removeIf(
                        e -> !(e instanceof CommandPolice || e instanceof CommandFire || e instanceof CommandAmbulance
                                || e instanceof MessageFireBrigade || e instanceof MessageAmbulanceTeam));
            }
    ...
    protected int[] getChannelsByAgentType(StandardEntityURN agentType, AgentInfo agentInfo,
        ...
        for (int i = 0; i < maxChannelCount; i++) {
            channels[i] = SEUChannelSubscriber.getChannelNumber(agentType, i, numChannels, agentInfo, worldInfo,
                    scenarioInfo);
        }
        return channels;
    }
    ...
    protected void setSendMessages(ScenarioInfo scenarioInfo, StandardEntityURN agentType, AgentInfo agentInfo,
        ...
        int allocatedCapacity = (int) (channelCapacity /
                (SEUChannelSubscriber.getScenarioAgents(scenarioInfo)
                        * SEUChannelSubscriber.getSendMessageAgentsRatio()));
        switch (agentType) {
            case FIRE_BRIGADE:
            case FIRE_STATION:
    ...
    private boolean allowedToSendRadioMessage(WorldInfo worldInfo, AgentInfo agentInfo, ScenarioInfo scenarioInfo) {
        ...
        Collection<StandardEntity> objectIDsInRange = worldInfo.getObjectsInRange(agentInfo.getID(),
                scenarioInfo.getPerceptionLosMaxDistance());
        for (StandardEntity entity : objectIDsInRange) {
            if (entity instanceof Human && !(entity instanceof Civilian)) {
                ...
                boolean inDistanceThreshold = worldInfo.getDistance(agentInfo.getID(),
                        entity.getID()) < ALLOWED_TO_SEND_DISTANCE_THRESHOLD;
                boolean inSameAreaAndSeenRange = agentInfo.getPosition().equals(worldInfo.getPosition(entity.getID()))
                        &&
                        worldInfo.getDistance(agentInfo.getID(),
                                entity.getID()) < scenarioInfo.getPerceptionLosMaxDistance() * 0.5;
