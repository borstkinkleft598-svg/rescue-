...
public class DefaultCommandExecutorAmbulance extends CommandExecutor<CommandAmbulance> {
    ...
    public DefaultCommandExecutorAmbulance(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
            DevelopData developData) {
        super(ai, wi, si, moduleManager, developData);
        this.type = ACTION_UNKNOWN;
        switch (scenarioInfo.getMode()) {
            case PRECOMPUTATION_PHASE:
                this.pathPlanning = moduleManager.getModule("DefaultCommandExecutorAmbulance.PathPlanning",
                        "AndroidRoboTeam.module.algorithm.AStarPathPlanning");
                this.actionTransport = moduleManager.getExtAction("DefaultCommandExecutorAmbulance.ExtActionTransport",
                        "AndroidRoboTeam.extaction.at.ActionTransport");
                this.actionExtMove = moduleManager.getExtAction("DefaultCommandExecutorAmbulance.ExtActionMove",
                        "AndroidRoboTeam.extaction.ActionExtMove");
                break;
            case PRECOMPUTED:
                this.pathPlanning = moduleManager.getModule("DefaultCommandExecutorAmbulance.PathPlanning",
                        "AndroidRoboTeam.module.algorithm.AStarPathPlanning");
    ...
    public CommandExecutor calc() {
        ...
        switch (this.type) {
            case ACTION_REST:
                if (DEBUG)
                    System.out.println(this.agentInfo.getID() + ": ACTION_REST");
            ...
            case ACTION_MOVE:
                if (DEBUG)
                    System.out.println(this.agentInfo.getID() + ": ACTION_MOVE");
                if (this.target != null) {
            ...
            case ACTION_RESCUE:
                if (DEBUG)
                    System.out.println(this.agentInfo.getID() + ": ACTION_RESCUE");
                if (this.target != null) {
            ...
            case ACTION_LOAD:
                if (DEBUG)
                    System.out.println(this.agentInfo.getID() + ": ACTION_LOAD");
                if (this.target != null) {
                    this.result = this.actionTransport.setTarget(this.target).calc().getAction();
                }
                return this;
            case ACTION_UNLOAD:
                if (DEBUG)
                    System.out.println(this.agentInfo.getID() + ": ACTION_UNLOAD");
                if (this.target != null) {
                    this.result = this.actionTransport.setTarget(this.target).calc().getAction();
                }
                return this;
            case ACTION_AUTONOMY:
                if (DEBUG)
                    System.out.println(this.agentInfo.getID() + ": ACTION_AUTONOMY");
                if (this.target != null) {
            ...
            case ACTION_LOAD:
                ...
                if (human1.isPositionDefined()) {
                    ...
                    if (this.worldInfo.getEntityIDsOfType(AMBULANCE_TEAM).contains(position)) {
                        ...
                        return this.worldInfo.getEntity(position).getStandardURN() == REFUGE;
                }
                return false;
            case ACTION_UNLOAD:
                if (this.target != null) {
            ...
            case ACTION_AUTONOMY:
                if (this.target != null) {
                    ...
                    if (targetEntity instanceof Area) {
                        ...
                        return this.isCommandCompleted();
                    } else if (targetEntity instanceof Human h) {
