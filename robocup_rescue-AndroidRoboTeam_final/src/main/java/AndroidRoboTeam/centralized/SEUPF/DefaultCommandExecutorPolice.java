package AndroidRoboTeam.centralized.SEUPF;
...
public class DefaultCommandExecutorPolice extends CommandExecutor<CommandPolice> {
	...
	public DefaultCommandExecutorPolice(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
		...
		switch (scenarioInfo.getMode()) {
			case PRECOMPUTATION_PHASE:
				this.pathPlanning = moduleManager.getModule("CommandExecutorPolice.PathPlanning",
						"adf.core.sample.module.algorithm.SamplePathPlanning");
				this.actionExtClear = moduleManager.getExtAction("CommandExecutorPolice.ActionExtClear",
						"adf.core.sample.extaction.ActionExtClear");
				this.actionExtMove = moduleManager.getExtAction("CommandExecutorPolice.ActionExtMove",
				...
				if (targetEntity.getStandardURN() == REFUGE) {
					...
					if (agent.getDamage() > 0) {
						if (position.getValue() != this.target.getValue()) {
							List<EntityID> path = this.pathPlanning.getResult(position, this.target);
							if (path != null && path.size() > 0) {
								Action action = this.actionExtClear.setTarget(path.get(path.size() - 1)).calc()
										.getAction();
								if (action == null) {
									action = new ActionMove(path);
								}
