...
public class DefaultCommandExecutorScout extends CommandExecutor<CommandScout> {
    ...
    public DefaultCommandExecutorScout(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
            DevelopData developData) {
        super(ai, wi, si, moduleManager, developData);
        this.type = ACTION_UNKNOWN;
        switch (scenarioInfo.getMode()) {
            case PRECOMPUTATION_PHASE:
                this.pathPlanning = moduleManager.getModule("CommandExecutorScout.PathPlanning",
    ...
    public CommandExecutor setCommand(CommandScout command) {
        ...
        if (command.isToIDDefined() && Objects.requireNonNull(command.getToID()).getValue() == agentID.getValue()) {
            EntityID center = command.getTargetID();
            if (center == null) {
                center = this.agentInfo.getPosition();
            }
            this.type = ACTION_SCOUT;
            this.commanderID = command.getSenderID();
            this.scoutTargets = new HashSet<>();
            this.scoutTargets.addAll(
                    this.worldInfo.getObjectsInRange(center, command.getRange())
                            .stream()
                            .filter(entity -> entity instanceof Area && entity.getStandardURN() != REFUGE)
    ...
    public CommandExecutor calc() {
        this.result = null;
        if (this.type == ACTION_SCOUT && this.scoutTargets != null && !this.scoutTargets.isEmpty()) {
