package AndroidRoboTeam.extaction;
...
public class DefaultExtActionMove extends ExtAction {
    ...
    private StuckHumans stuckHumans;

    private boolean stuck = false;

    private int lastStuckUpdateTime = -1;

    public DefaultExtActionMove(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo,
        ...
        this.stuckHumans = new StuckHumans(world, worldInfo);
    ...
    private Action moveOnPath(List<EntityID> path) {
        ...
        if (stuckFlag) {
            action = stuckHumans.calc(path);
            if (SEUConstants.DEBUG_STUCK_HELPER && action == null) {
                System.out.println(world.getSelfHuman().getID() + " stuckService fail to path(0)" + path.get(0));
            }
        }
    ...
    public boolean isStuck(List<EntityID> path) {
        ...
        if (lastStuckUpdateTime == agentInfo.getTime()) {
            ...
            int moveDistance = getDistance(position, this.lastPosition);
