...
public class SEUExtActionClearOld extends ExtAction {
    ...
    public SEUExtActionClearOld(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
    ...
    private Action rescueSurroundings(PoliceForce pf, Area pfArea) {
        ...
        for (Human human : humans) {
            ...
            if (!stuckArea.isEmpty()) {
                Point2D humanPoint = this.getPoint(human);
                Action a = this.clearPoint(this.getPoint(), humanPoint, pf,
                        (Road) humanArea, true);
                if (a != null) {
                    if (SEUConstants.PF_ACTION_EXT_CLEAR_LOG) {
    ...
    private Action clearPoint(Point2D agentPoint, Point2D goal, Human human, Road road,
        ...
        Collection<Blockade> blockades = this.getPath(true, this.getMyself().getID(), road.getID())
                ...
                .filter(Blockade::isApexesDefined)
                .collect(toSet());
        Point2D intersect = this.getNearestIntersectPoint(agentPoint, agentPoint, goal, blockades);
        if (intersect != null) {
            return this.clearPoint(agentPoint, intersect, blockades, human, road, isMove);
        } else {
