...
public class SEUExtActionClearNew extends ExtAction {
    ...
    public SEUExtActionClearNew(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager,
    ...
    private Action clearAndMoveByConcretePathExpand(PoliceForce pf, List<EntityID> path, int agentRadius) {

        if (path == null || path.size() <= 2) {
            if (SEUConstants.PF_ACTION_EXT_CLEAR_LOG) {
                personalLogger.debug("[ERROR]: clearAndMoveByConcretePathExpand");
            }
            return null;
        }
        ...
        for (Line2D line2D : concretePath) {
            ...
            java.awt.geom.Area expandedLineArea = this.expandLine2DToAWTArea(concreteLine, agentRadius);
            Collection<Blockade> overlappingBlockades = blockades.stream()
                    .filter(Blockade::isApexesDefined)
                    .filter(b -> {
                        java.awt.geom.Area overlap = (java.awt.geom.Area) expandedLineArea.clone();
                        overlap.intersect(new java.awt.geom.Area(b.getShape()));
                        return !overlap.isEmpty();
                    })
                    .collect(toSet());

            Point2D intersect = this.getNearestIntersectPoint(concreteLine.getOrigin(), concreteLine.getOrigin(),
                    concreteLine.getEndPoint(), overlappingBlockades);

            if (intersect == null && !overlappingBlockades.isEmpty()) {
                Blockade nearest = overlappingBlockades.stream()
                        .min(comparing(b -> this.worldInfo.getDistance(pf, b)))
                        .orElse(null);
                intersect = this.getClosestPointToBlockade(nearest);
            }
    ...
    private Action rescueSurroundings(PoliceForce pf, Area pfArea) {
        ...
        for (Human human : humans) {
            ...
            if (this.isHumanStuckInBlockade(human)) {
                Action a = this.moveToPoint(this.getPoint(), new Point2D(human.getX(), human.getY()), pf,
                        (Road) humanArea, true);
                if (a != null) {
                    if (SEUConstants.PF_ACTION_EXT_CLEAR_LOG) {
    ...
    private Action clearOrMoveToNearbyArea(PoliceForce pf, Area target, boolean isMove) {
        ...
        if (target instanceof Road road) {
            ...
            if (!road.isBlockadesDefined() || this.isRoadGuidelinePassable(road)) {
                return new ActionMove(Lists.newArrayList(posEntity.getID(), target.getID()));
            }
    ...
    private Action moveToPoint(Point2D agentPoint, Point2D goal, Collection<Blockade> blockades, Human human, Road road,
        ...
        if (intersectPoint != null) {
            ...
            if (distance < this.CLEAR_REPAIR_DISTANCE * this.CLEAR_ALLOWANCE_RATIO) {
                ...
                Action action;
                if (this.actionHistory == null || this.actionHistory.isEmpty()) {

                    action = new ActionClear(clearScaleX, clearScaleY);
                    return action;
                }

                Action lastAction = this.actionHistory.getLast();
                if (!(lastAction instanceof ActionClear lastClearAction)) {

                    action = new ActionClear(clearScaleX, clearScaleY);
                    return action;
                }

                Point2D lastClearPoint = new Point2D(lastClearAction.getPosX(), lastClearAction.getPosY());
                if (isPoint2DNearPoint2D(new Point2D(0, 0), lastClearPoint)
                        || !isPoint2DNearPoint2D(new Point2D(clearScaleX, clearScaleY), lastClearPoint)) {
    ...
    private Action clearPoint(Point2D agentPoint, Point2D goal, Collection<Blockade> blockades, Human human, Road road,
        ...
        if (distance < this.CLEAR_REPAIR_DISTANCE * this.CLEAR_ALLOWANCE_RATIO) {
            ...
            if (isPoint2DNearPoint2D(new Point2D(0, 0), lastClearPoint)
                ...
                if (SEUConstants.PF_ACTION_EXT_CLEAR_LOG) {
                    personalLogger.debug(
                            "[ERROR]: CanNotDetectPointHasCleared: " + "(clearGoalX, clearScaleY) = " + clearGoalX
                                    + ", " + clearGoalY);
                }
    ...
    private Action makeActionToAvoidError() {
        ...
        if (blockade != null) {
            ...
            return this.clearPoint(myPoint, clearPoint, Lists.newArrayList(blockade), this.getMyself(),
